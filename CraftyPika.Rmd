...
### <b>tokenize_tbl</b>(tbl, token='words', n=NULL, excluded=NULL, ...)
```{r}
...
```
# 
# exclude_words <- rbindlist(list(stop_words[stop_words$lexicon=='snowball', 1], 
#                                 profanity))
...
### <b>Tokenize</b>
```{r}
if (!exists(train)){load(paste0(object_dir,'/clean_train.rda'))}
```
```{r}
f
token_counts <- tokenize_docs(docs, 
                              token='ngrams', 
                              n=ngram_length, 
                              excluded=profanity)
```
